...
const (
	defaultStartupProgressDelay    = 150 * time.Millisecond
	defaultStartupProgressInterval = 180 * time.Millisecond
)
...
func (p *StartupProgress) run() {
	...
	rendered = true
	ticker := time.NewTicker(defaultStartupProgressInterval)
	defer ticker.Stop()
	for {
		select {
		case <-p.stop:
