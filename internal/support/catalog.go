...
func Catalog() []Entry {
	entries := []Entry{
		...
		{Surface: "provider", Name: "mise", Label: LabelSupportedPreview, Summary: "mise runtime/tool inventory, update, bump, manifest hygiene, and backend convergence evidence.", Scope: "developer CLI runtimes and tools", Evidence: []string{"mise JSON contracts", "v2026.8.2 bootstrap status/plan and package dry-run probes", "minimum-release-age diagnostics", "backend metadata resolvers"}, Limitations: []string{"opaque backends remain review-held"}},
		...
		{Surface: "command", Name: "doctor package-executors", Label: LabelExperimental, Summary: "Read-only native/mise executor decisions from desired-source, platform, and capability evidence.", Evidence: []string{"macOS x64/arm64 and Linux fixtures", "JSON and explicit TUI"}, Limitations: []string{"does not generate commands or apply packages"}},
		{Surface: "command", Name: "doctor package-parity", Label: LabelExperimental, Summary: "Read-only comparison of the active Brewfile and mise bootstrap package projection.", Evidence: []string{"mise bootstrap status JSON", "active mise config sources", "formula/cask/tap canonical identities"}, Limitations: []string{"reports drift only; does not select an executor or mutate desired state"}},
		...
		{Surface: "report", Name: "package-parity", Label: LabelExperimental, Summary: "Formula, cask, and tap parity between Brewfile authority and the read-only mise projection."},
		{Surface: "report", Name: "package-executors", Label: LabelExperimental, Summary: "Capability- and architecture-driven package executor decisions without command execution."},
