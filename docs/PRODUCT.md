...
| [DESIGN.md](DESIGN.md) | TUI visual identity, semantic styles, component appearance, and visual baselines. |
| [UX.md](UX.md) | Interactive routes, focus/return behavior, review flows, and performance contract. |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Ownership boundaries, provider model, package layout, shared-internal extraction rules. |
| [CLI.md](CLI.md) | Command surface, output-mode selection, text/JSON shape, TTY entrypoints, and localization. |
...
|-------------|-------|--------------------|
| Product boundary, stable scope, and source-of-truth policy | `PRODUCT.md` | README may summarize the value proposition; release docs may point back here. |
| Interactive route, focus, state, and action behavior | `UX.md` and `ux/` | CLI docs describe entry flags only; visual appearance belongs in `DESIGN.md`. |
| TUI visual identity, semantic styles, component appearance, visual baselines | `DESIGN.md` | Validation owns executable commands and pinned render inputs. |
| Command surface, flags, exit codes, output-mode selection, text/JSON shape, TTY entrypoints | live `updev help` and `CLI.md` | README and agent docs may show short recipes only. Stateful TTY behavior belongs in `UX.md`. |
...
- `updev backends doctor` and `updev backends plan` report Homebrew-to-mise and
  mise backend convergence opportunities without mutation. Registry-backed
  recommendations come from a bounded mise registry snapshot plus an
  item-scoped platform dry-run. Interactive detail views may add a pinned mise
  desired entry or remove superseded ownership only as separate confirmed
  actions; they never uninstall Homebrew packages automatically.
